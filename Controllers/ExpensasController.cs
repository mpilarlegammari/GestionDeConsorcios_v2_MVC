...
using GestionDeConsorcios_v2_MVC.Models.ViewModels;
...
namespace GestionDeConsorcios_v2_MVC.Controllers
{
    public class ExpensasController : Controller
    {
        ...
        // Acción agregada: Resumen por consorcio y período
        public async Task<IActionResult> IndexResumen(int? consorcioId, string periodo)
        {
            // Incluir UnidadFuncional y su Consorcio para acceder al Consorcio desde la Expensa
            ...
            if (consorcioId.HasValue)
                expensasQuery = expensasQuery.Where(e => e.UnidadFuncional != null && e.UnidadFuncional.ConsorcioId == consorcioId.Value);

            if (!string.IsNullOrEmpty(periodo))
                expensasQuery = expensasQuery.Where(e => e.Periodo == periodo);
            // Traer expensas a memoria y agrupar por Consorcio+Periodo para resumen
            var expensasList = await expensasQuery
                .Where(e => e.UnidadFuncional != null && e.UnidadFuncional.ConsorcioId != 0)
                .ToListAsync();

            var resumen = expensasList
                .GroupBy(e => new { ConsorcioId = e.UnidadFuncional.ConsorcioId, e.Periodo })
                .Select(g => new ExpensaPeriodoRowViewModel
                {
                    ConsorcioId = g.Key.ConsorcioId,
                    ConsorcioNombre = g.First().UnidadFuncional?.Consorcio?.Nombre,
        ...
        public async Task<IActionResult> DetalleExpensa(int? consorcioId, string periodo)
        {
            ...
            if (!string.IsNullOrEmpty(periodo))
            {
                ...
                ViewBag.FilterPeriodo = periodo;
            }

            var expensas = await query
                .OrderByDescending(e => e.Periodo)
                .ThenBy(e => e.UnidadFuncionalId)
                .ToListAsync();
